<template>
<div>
    <h1>Exercice NodeJS/VueJS M1</h1>
    <br>
    <button v-on:click="isActive = !isActive" style="display: none;">Bouton</button>
    <div>
    <form action="http://localhost:8080/#/tJS">
    <input type="submit" value="Exercice ThreeJS" />
    </form>
    </div>
    
  


  <button style="display: none;" @click="test()">Try it</button>
  <br>
  <br>
  
    <div style="background-image: url(https://images2.alphacoders.com/116/thumb-1920-1160159.png); height: 65em; ">
        <div id="form" style="background-color: rgba(255, 255, 255, 0.40); display: flex; flex-direction: column; box-shadow: 0px 3px 2px rgba(89, 89, 89, 0.5);">
              <div style="font-family: 'Pokemon Solid', sans-serif;">
              <h1 style="margin-top: 0;">Trouves ton Pokemon !</h1>
              <p style="margin-top: -2em;">Entres ton chiffre fétiche et découvres à quel Pokemon celui-ci correspond !</p>
              </div>
              <div style="padding: 1em;   width: 80%;display: flex; margin: 1px auto;text-align: center; flex-direction: column;">
              <form style="display: flex; flex-direction: column; width: 10em; align-self: center;">
                  <input type="number" id="pokemon" style="padding: 0.5em;" placeholder="(1 - 898)"/>
                  <input type="button" class="pokebtn" @click="searchPokemon()"  style="width: 5em; align-self: center; margin-top: 1em;">
              </form>
              </div>
          </div>
          <div style="display: flex; padding: 2.5em;">
          <div id="result" style="background-color: rgba(255, 255, 255, 0.85); margin: auto; padding: 2em;border-radius: 10% / 50%; display: none; font-family: 'Pokemon Solid', sans-serif; box-shadow: 1px 1px 40px 1px rgb(0 0 255 / 80%);"></div>
          </div>
    </div>
  </div>
</template>

<script>
export default {
  name: 'HelloWorld',
  data () {
    return {
      msg: 'Welcome to Your Vue.js App',
      isActive: true
    }
  },methods :{
    test: function () {
  var img = document.createElement("img");
  img.src = 'data:image/jpeg;base64,';
  document.body.appendChild(img);
},
    searchPokemon: function (){

        var idpok = $("#pokemon").val();

        if (idpok == null || idpok == "" || idpok == 0) {
         alert("N'oublies pas de rentrer ton chiffre favori ;) !");
         return false}

        if (idpok > 898) {
         alert("Il n'existe pas autant de Pokemon !");
         return false}

         if (idpok < 0) {
         alert("Pas à nous :)");
         return false}

        if (idpok % 1 == 0) {} 
        else {
        alert("N'oublies pas de rentrer ton chiffre favori ;) !");
        return false
        }

        $('#result').empty();
        var data = $.getJSON("https://pokeapi.co/api/v2/pokemon/"+idpok, function(data) {
            $('#result').append('<h1>Voici votre Pokemon</h1>');
            $('#result').append('<p> Son Nom : ' + data.name + '</p>');  
            $('#result').append('<p> Son Poids : ' + data.weight + ' Kg</p>'); 
            $('#result').append('<p>Type(s) :</p>');
            for (let key of data.types) {
                $('#result').append('<p> -> ' + key.type.name + '</p>');
            };
            $('#result').append('<img style="height: auto;width: 60%; margin-bottom: -2em;" src= '+ data.sprites["front_default"] +' > </img>'); 
        });
          var x = document.getElementById("result");
          if (x.style.display === "none") {
          x.style.display = "block";
          } else {
          x.style.display = "block";
          }
},
}
  



}

</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
h1, h2 {
  font-weight: normal;
  font-family: 'Pokemon Solid', sans-serif;
}
ul {
  list-style-type: none;
  padding: 0;
}
li {
  display: inline-block;
  margin: 0 10px;
}
a {
  color: #42b983;
}

.pokebtn{
                display: inline-block;
                outline: 0;
                border:0;
                cursor: pointer;
                text-decoration: none;
                position: relative;
                color: #000;
                background: url("https://archive.org/download/PokemonIcon/pokemon%20icon.png");
                background-size: 35px;
                background-repeat: no-repeat;
                background-position: center;
                line-height: 30px;
                border-radius: 40px;
                padding: 5px;
                font-size: 20px;
                font-family: 'Pokemon Solid', sans-serif;
                font-weight: 600;
                box-shadow: rgb(153, 0, 0) -2px -2px 0px 2px, rgb(255, 255, 255) 0px 0px 0px 4px, rgba(0, 0, 0, 0.05) 0px 0px 2px 7px;
                transition: all 0.2s;
}
.pokebtn:hover{
                    box-shadow: rgb(255, 255, 255) -2px -2px 0px 2px, rgb(153, 0, 0) 0px 0px 0px 4px, rgba(0, 0, 0, 0.05) 0px 0px 2px 7px;
                    transform: scale(1.00);
                }

::-webkit-input-placeholder {
   text-align: center;
   margin-right: -10px;
}                

@import url('http://fonts.cdnfonts.com/css/pokemon-solid');
</style>
